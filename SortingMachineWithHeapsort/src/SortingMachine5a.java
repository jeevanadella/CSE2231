...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        //if the right tree exists the left tree does as well
        if (right <= last) {
            //assign root and children
            T root = array[top];
            T nodeOne = array[2 * top + 1];
            T nodeTwo = array[2 * top + 2];
            //check to see if nodeOne is less than or equal to
            //nodeTwo and see if root is less than nodeTwo
            if (order.compare(nodeOne, nodeTwo) <= 0
                    && order.compare(root, nodeTwo) > 0) {
                exchangeEntries(array, top, left);
                siftDown(array, left, last, order);
            }
            //check to see if nodeTwo is less than or equal to
            //nodeOne and see if root is less than nodeOne
            if (order.compare(nodeTwo, nodeOne) <= 0
                    && order.compare(root, nodeTwo) > 0) {
                exchangeEntries(array, top, right);
                siftDown(array, right, last, order);
            }
            //check to see there is no right tree and a left tree
        } else if (left <= last && right > last) {
            T root = array[top];
            T nodeOne = array[left];
            //check to see if the left child is less than root
            if (order.compare(nodeOne, root) < 0) {
                exchangeEntries(array, top, left);
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        int left = 2 * top + 1;
        int right = 2 * top + 2;
        //if there is a right tree there is a left
        if (right < array.length) {
            //heapify left and right tree
            heapify(array, left, order);
            heapify(array, right, order);
            //check to see if there is a left tree
    ...
    public final T removeFirst() {
        ...
        // changes root val to second value of orginal heap
        this.heap[0] = this.heap[this.heapSize - 1];
        // update size
        this.heapSize--;

        // sift down to reformat rest of heap accordingly
