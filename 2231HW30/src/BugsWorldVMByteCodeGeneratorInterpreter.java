 ...
 *
 */
public final class BugsWorldVMByteCodeGeneratorInterpreter {

    /**
     * No argument constructor--private to prevent instantiation.
     */
    private BugsWorldVMByteCodeGeneratorInterpreter() {
    }
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
        ...
        while (location < cp.length && notFound) {

            if (isPrimitiveInstructionByteCode(cp[location])) {
                notFound = false;
            } else if (cp[location] == Instruction.JUMP.byteCode()) {
                location = cp[location + 1];
            } else {
                if (!conditionalJumpCondition(wbs, cp[location])) {
                    location = cp[location + 1];
                } else {
                    location += 2;
                }
