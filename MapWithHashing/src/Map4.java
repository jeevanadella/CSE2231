...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    public final void add(K key, V value) {
        ...
        //Hash to find the find the right bucket for the key
        int hash = key.hashCode();
        int bucket = mod(hash, this.hashTable.length);

        //Add key and value to the hashed bucket
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // find associated bucket
        int hash = key.hashCode();
        int bucket = mod(hash, this.hashTable.length);

        // remove pair from bucket
        Map.Pair<K, V> removal = this.hashTable[bucket].remove(key);
    ...
    public final Pair<K, V> removeAny() {
        ...
        //iterate to the first non empty bucket
        int i = 0;
        ...
        //remove any on the non  empty bucket
        Map.Pair<K, V> removal = this.hashTable[i].removeAny();
        //decrease size of object
        this.size--;
    ...
    public final boolean hasKey(K key) {
        ...
        //Hash to find the find the right bucket for the key if it exists
        int hash = key.hashCode();
        int bucket = mod(hash, this.hashTable.length);

        //check to see if the key is in the bucket
