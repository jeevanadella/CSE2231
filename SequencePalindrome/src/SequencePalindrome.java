...
//    /**
//     * Checks if a given {@code Sequence<Integer>} is a palindrome iteratively.
//     *
//     * @param s
...
//            palindrome = true;
//        } else {
//            while (!((seqLen == 0) || (seqLen == 1)) && (palindrome)) {
//                int first = s.entry(bottom);
//                int last = s.entry(seqLen - 1);
//                if (first != last) {
//                    palindrome = false;
//                }
//                bottom += 1;
...
//    }
    ...
    /**
     * Checks if a given {@code Sequence<Integer>} is a palindrome recursively.
     *
     * @param s
     *            the {@code Sequence} to check
     * @return true if the given {@code Sequence} is a palindrome, false
    ...
    private static boolean isPalindrome(Sequence<Integer> s) {
        ...
        if ((seqLen == 0) || (seqLen == 1)) {
            ...
            while (!((seqLen == 0) || (seqLen == 1)) && (palindrome)) {
                int last = s.remove(seqLen - 1);
                int first = s.remove(bottom);
                if (first != last) {
                    palindrome = false;
                }
                bottom += 1;
