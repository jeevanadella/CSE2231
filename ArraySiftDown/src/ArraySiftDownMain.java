 ...
 * </pre>
 *
 * @author Adam Khalil, Jeevan Nadella
 *
 */
...
//            if ((left <= last)) {
//                smaller = left;
//                //if left child check if right child
//                if (right <= last) {
//                    //if right smaller make it smaller
...
//
//                //switch placement
//                if (array[smaller] < array[top]) {
//                    place = array[top];
//                    array[top] = array[smaller];
//                    array[smaller] = place;
//                    //if left is the one that was switched siftdown left
//                    if (smaller == left) {
...
//        }
        ...
        if (last - top > 1) {
            if (left < last && array[left] < array[root]) {
                place = array[top];
                array[top] = array[left];
                array[left] = place;
                root = left;
            }
            if (right < last && array[right] < array[top]) {
                place = array[top];
                array[top] = array[right];
                array[right] = place;
                root = right;
            }
            if (root != top) {
                //array.exchangeEntries(top, root);
                siftDown(array, root, last);
            }
        }
